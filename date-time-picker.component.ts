...
export class DateTimePickerComponent implements OnInit, OnDestroy, ControlValueAccessor {
    ...
    private inputValueChanged = false; // a flag to indicate if the text input value was changed
    ...
    private alignDialog(): void {
        const element = this.dialogElm.nativeElement;
    ...
    private generateCalendar(): void {
        ...
        this.calendarDays = [];
        const startDateOfMonth = startOfMonth(this.pickerMoment);
        const startWeekdayOfMonth = getDay(startDateOfMonth);
    ...
    private updateFormattedValue(): void {
        ...
        if (this.value) {
            if (this.isSingleSelection()) {
                formattedValue2 = format(this.value, this.dateFormat, {locale: this.locale.dateFns});
            } else if (this.isRangeSelection()) {
                const startDate = this.value[0];
                const endDate = this.value[1];

                formattedValue2 = format(startDate, this.dateFormat, {locale: this.locale.dateFns});

                if (endDate) {
                    formattedValue2 += ' - ' + format(endDate, this.dateFormat, {locale: this.locale.dateFns});
                } else {
                    formattedValue2 += ' - ' + this.dateFormat;
                }
            } else if (this.isMultiSelection()) {
                for (let i = 0; i < this.value.length; i++) {
                    const dateAsString = format(this.value[i], this.dateFormat, {locale: this.locale.dateFns});
        ...
        this.formattedValue = formattedValue2;
